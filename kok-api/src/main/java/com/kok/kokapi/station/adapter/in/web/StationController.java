...
public class StationController {
    ...
    public ResponseEntity<ApiResponseDto<List<RecommendedStationResponse>>> searchStations(@PathVariable String keyword) {

        List<RecommendedStationResponse> recommendedStations = customStationUseCase.searchStations(keyword).stream()
                .map(station ->
                        RecommendedStationResponse.of(station, retrieveRouteUseCase.retrieveRoutes(station)))
                .toList();
    ...
    public ResponseEntity<ApiResponseDto<RecommendedStationResponse>> addCustomStations(@PathVariable String roomId, @PathVariable Long stationId) {
        Station station = customStationUseCase.addCustomStations(roomId, stationId);
        return ResponseEntity.ok(ApiResponseDto.success(RecommendedStationResponse.of(station, retrieveRouteUseCase.retrieveRoutes(station))));
    }
    ...
    public ResponseEntity<ApiResponseDto<List<RecommendedStationResponse>>> getCustomRecommendedStations(@PathVariable String roomId) {
        List<RecommendedStationResponse> candidateStations = stationFacadeService.getCandidateStationResponse(roomId);
